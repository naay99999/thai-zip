...
| `thaizip/data` | `loadDefaultIndex`, `clearDefaultIndex`, `getDefaultIndexIfLoaded` (2.3 KB gzip + ก้อนข้อมูลอีก 114 KB gzip ที่ดึงทีหลัง) |
...
ตัวชุดข้อมูลเองยังถูกแยกเป็นไฟล์ของมันเองอีกชั้น ดังนั้น bundler ที่แยก chunk ตาม dynamic import ได้ (webpack, Turbopack, Vite/Rollup) จะยังไม่ดาวน์โหลด 114 KB นั้นจนกว่าจะเรียก `loadDefaultIndex()` จริง ๆ — แค่ import `thaizip/data` เฉย ๆ ยังไม่ทำให้โหลด ทั้งนี้เป็นการรับประกันแบบ best-effort: bundler หรือโหมด build บางแบบ (เช่น SSR/CJS บางกรณี) ที่ inline dynamic import ทั้งหมดจะยังรวมมันเข้า chunk แรกอยู่ดี
